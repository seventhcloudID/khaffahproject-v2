...
interface ProductHeadlineProps {
  ...
  /** Slug dari URL (mis. umrah-reguler-maret). Dipakai untuk link checkout agar tidak hardcode. */
  slug?: string;
}

const ProductHeadline = ({ paketUmrahId = "1", slug: slugProp }: ProductHeadlineProps) => {
  const pathname = usePathname();
  const { data } = useUmrahById(paketUmrahId);
  const product = data?.data?.data;

  // Slug untuk link checkout: dari prop (halaman [slug]) atau dari pathname atau dari nama produk
  const checkoutSlug =
    slugProp ||
    (pathname ? pathname.replace(/^\//, "").split("/")[0] : null) ||
    (product?.nama_paket ? productNameToSlug(product.nama_paket) : null);
  const checkoutHref = checkoutSlug ? `/${checkoutSlug}/checkout` : "#";
      ...
      <div className="bg-white p-8 rounded-2xl gap-4 flex flex-col md:flex-row justify-between lg:justify-between">
        ...
        <div className="flex gap-2 flex-col-reverse md:flex-col lg:flex-col xl:flex-row ">
          ...
          <div className="flex flex-row-reverse justify-between md:justify-end md:flex-col lg:flex-row-reverse gap-2">
            <Link href={checkoutHref} prefetch={!!checkoutSlug}>
