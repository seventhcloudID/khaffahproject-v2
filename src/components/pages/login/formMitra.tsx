...
import { loginSchema } from "@/typing/login";

export default function LoginFormMitra() {
  const [showPassword, setShowPassword] = useState(false);
  const { mutate: login, isPending } = useLogin({ checkRole: "mitra" });
  ...
  return (
    <div className="mx-auto max-w-[1120px] lg:max-w-[1180px] grid grid-cols-1 lg:grid-cols-[500px_minmax(0,1fr)] items-stretch gap-0 w-full">
      <div className="flex justify-center lg:justify-end w-full">
        <div className="w-full max-w-[500px] space-y-4 p-6 bg-white md:rounded-xl lg:rounded-l-2xl lg:rounded-r-none shadow">
          ...
          <p className="text-sm text-center text-gray-500">
            Akses akun mitra Anda dan lanjutkan aktivitas usaha anda
          </p>
          ...
          <Form {...form}>
            ...
            >
              ...
              <Button
                type="submit"
                className="w-full bg-khaffah-primary text-white"
                disabled={isPending}
              >
                {isPending ? (
           ...
           <Button
            ...
            aria-label="Masuk sebagai User"
          >
            <Link href="/join-partner/fill-form">
              <User className="w-4 h-4 text-khaffah-secondary" />
              <span>Daftar Mitra</span>
            </Link>
          </Button>
          <div className="flex items-center gap-3 text-black/50">
            <hr className="flex-1 border-black/10" />
            <p className="text-xs whitespace-nowrap">Atau lanjutkan dengan</p>
            <hr className="flex-1 border-black/10" />
          </div>
      ...
      <div className="hidden lg:block">
        <div className="relative h-[520px] xl:h-[560px] w-[520px] xl:w-[560px] rounded-r-2xl overflow-hidden">
          <Image
            src={"/assets/kabah.jpg"}
            alt="makkah-view"
            fill
            priority
